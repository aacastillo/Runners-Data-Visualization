...
function makeVisualization(attributes) {
    ...
    if (attributes.size === 1) {
        const [a1] = attributes;
        if (attributeType[a1] === "categorical") return buildBarChart(a1, vis_div);
        if (attributeType[a1] === "quantitative") return buildTrendGraph(a1, vis_div);
        console.log("ERROR: invalid attribute selected or no type found");
    } else if (attributes.size === 2) {
        const [a1, a2] = attributes;
        if (attributes.size === 1 && attributeType[a1] === "categorical") return buildClusterBarChart(a1, a2, vis_div);
        if (attributes.size === 1 && attributeType[a1] === "quantitative") return buildScatterPlot(a1, a2, vis_div);
        //Note: when making a whisker plot, make sure that a1, the first attribute passed, is categorical
        if (attributeType[a1] === "categorical" && attributeType[a2] ==="quantitative") return buildWhiskerPlot(a1, a2, vis_div);
...
function buildWhiskerPlot(a1, a2, vis_div) {
    ...
    d3.csv("https://raw.githubusercontent.com/aacastillo/Runners-Data/main/RunningDataset.csv", function(data) {
